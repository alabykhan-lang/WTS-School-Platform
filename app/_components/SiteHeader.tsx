  ...
  return (
    <header className="siteHeader">
      ...
      <div className="headerActions">
        <Link className="headerButton" href="/portal/sign-in" aria-current={isActive("/portal") ? "page" : undefined}>WTS Staff <span>Workspace</span></Link>
      ...
      <div id="mobile-navigation" className={`mobileNav ${open ? "isOpen" : ""}`} aria-hidden={!open}>
        <nav aria-label="Mobile navigation">
          ...
          <Link href="/portal/sign-in" tabIndex={open ? 0 : -1} className={isActive("/portal") ? "active" : undefined} aria-current={isActive("/portal") ? "page" : undefined} onClick={() => setOpen(false)}>WTS Staff Workspace <em>Open</em></Link>
        </nav>
      </div>
    </header>
  );
